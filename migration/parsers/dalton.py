    ...
    *,
    ...
    for j in range(start, len(lines)):
        ...
        if len(parts) == 5:
            ...
            start_float_index = 1
        label = parts[0].split("_", 1)[0]
        x, y, z = (float(parts[start_float_index]), float(parts[start_float_index + 1]), float(parts[start_float_index + 2]))
        atoms.append(label)
        coords.append([x, y, z])
    ...
    mol = qcel.models.Molecule(symbols=atoms, geometry=coords, orient=True, extras={"fix_orientation": False})
    return (mol, atoms, coords) if return_raw else mol
...
if __name__ == "__main__":
    ...
    basis = "d-aug-cc-pVQZ"
    dalton_file = Path(
        f"raw_data/raman_paper/debug/{mol}/dalton/raman_{mol}-{basis}.out"
    )

    # NumPy printing (optional)
    np.set_printoptions(
        precision=2, suppress=False, formatter={"float_kind": "{:0.2e}".format}
    )
    np.set_printoptions(linewidth=200)

    parser = DaltonParser(dalton_file).parse_all()
    print(f"Parsed Dalton output from {dalton_file}")
    print(parser.polarizability)

    # The rest is debug / inspection, currently unreachable due to sys.exit() above
    print(parser.molecule)
    print("Atoms:", parser.atoms)
    print("Coordinates (Å):", parser.coords)

    print("Hessian shape:", parser.hessian.shape)
    print(parser.hessian)
    print("Normal coordinates shape:", parser.normal_coordinates.shape)
    print(parser.normal_coordinates)

    print(f"Found {len(parser.vib_rows)} vibrational modes")
    for row in parser.vib_rows:
        print(row)

    print(
        "Polarizability Cartesian gradients at freqs (au):",
        sorted(parser.polar_cart_grad.keys()),
    )
    for f, M in parser.polar_cart_grad.items():
        print(f"[Cart Grad] f={f:.6f} → {M.shape}")
        print(M)

    print(
        "Polarizability normal-coordinate gradients at freqs (au):",
        sorted(parser.polar_normal_grad.keys()),
    )
    for f, M in parser.polar_normal_grad.items():
        print(f"[Normal Grad] f={f:.6f} → {M.shape}")
        print(M)

    for f_au, rows in parser.raman_by_freq.items():
        print(f"Raman @ {f_au:.6f} au : {len(rows)} modes")
        print("freq_cm1 | alpha^2 | beta(a)^2 | dep_ratio")
        for r in rows:
            print(
                f"{r['freq_cm1']:8.2f} | {r['alpha2']:8.6f} | {r['beta_a2']:9.6f} | {r['dep_ratio']:8.6f}"
            )
        print("---")

    print(f"Final Hartree–Fock Energy: {parser.hf_energy:.12f} Hartree")
    print("Summary:", parser.summary())

    orb = parser.parse_orbitals()
    print("Electrons:", orb["nelec"])
    print("HOMO:", orb["homo"], "LUMO:", orb["lumo"], "gap:", orb["gap"])
    for sym, eps in orb["energies_by_sym"].items():
        print(sym, eps)
